...
.venv/bin/python -m duckman.eval --policy learned --seed 2 --checkpoint "$CK" --video results/realtime_seed2.mp4 --speed 1 --max-t 30 \
    --label "Real time, unedited, seed 2 (first 30 s)"
.venv/bin/python -m duckman.eval --policy neutral --seed 0 --video results/neutral_clip.mp4 --speed 1 --max-t 40 \
