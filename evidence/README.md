...
|---|---|---|
| 60-seed benchmark table (learned / planner / neutral / frozen), paired difference, CI, sign test | `python -m duckman.bench --seeds 60` | `results/bench.json` (240 rows), `results/bench.md` |
...
| Seed-0 causality set: learned, planner, neutral, frozen (no network), generation-0 untrained (40 s window) | `./run.sh` | `results/learned_seed0.json`, `results/planner_seed0.json`, `results/neutral_seed0.json`, `results/frozen_seed0.json`, `results/gen0_seed0.json` |
...
| README result and benchmark tables (between the HTML comment markers) | `python -m duckman.report results/*_seed*.json` | `README.md` |
| Scoring two strategy checkpoints on the training pool (used to pick elites) | `python -m duckman.compare a.npz b.npz` | stdout |
